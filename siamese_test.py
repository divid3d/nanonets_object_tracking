...
net = torch.load('/contect/gdrive/MyDrive/deep_sort/models/model2.pt').cuda()
net.eval()
...
for i in range(total - 1):
    ...
    for j in range(len(negatives)):
        ...
        if d1 > d2:
            ...
            # correct+=1
            total_correct += 1
        print("Correct: {}%".format((batch_correct * 100.0) / (j + 1)))

    if batch_correct == len(negatives):
        correct += 1
    print("Correct batch: {}%".format((correct * 100.0) / (i + 1)))
